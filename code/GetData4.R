...
habitats <- rl_habitats("Theloderma asperum", key = token)
...
practice <- rl_habitats("Afrixalus clarkei", key = token)

en_frogs <- read.table(file = "en_frogs.txt", stringsAsFactors = FALSE)
en_frogs <- en_frogs$x
vu_frogs
...
nt_habitat <- sp_habitats(nt_frogs) #peNDING? 
lc_habitat <- sp_habitats(lc_frogs)



write.table(vu_threats, "/Users/Alexandra_Gonzalez/Desktop/Herpetology/vu_threats.txt", sep="\t")
nt_habitat2 <- read.table(file = "nt_habitat.txt", stringsAsFactors = FALSE)
...
practice10 <- habitat_table(en1_10)


species
practice <- "14.3 1.5 1.6"
hey <- strsplit(practice, split = " ")


######THIS FUNCTION: I'm trying to go through each habiat code and then populate the data table 
pop_habitats <- function(data){
  habitatdf <- habitat_table(data$species_list)
  i <- 1 
  for (string in data$habitat_code){
    many_strings <- strsplit(string, split = " ")
    for (char in many_strings){
      for (code in char){
        if (code == "1.1"){
          habitatdf$F1.1[i] <- 1
        } else if (code == "1.2"){
...
test <- pop_habitats(vu_habitat)       
head(vu_habitat)
vu_habitat <- read.table(file = "vu_habitat.txt", header = TRUE, stringsAsFactors = FALSE)
        
habitatdf <- habitat_table(en1_10)
code_vector <- c("1.2", "1.4", "1.5", "2.3", "1.8")
...
cr_habitat <- read.table(file = "cr_habitat.txt", header = TRUE, stringsAsFactors = FALSE)
en_habitat <- read.table(file = "en_habitat.txt", header = TRUE, stringsAsFactors = FALSE)
vu_habitat <- read.table(file = "vu_habitat.txt", header = TRUE, stringsAsFactors = FALSE)
...
nt_habitat_table$category <- "NT"
lc_habitat_table$category <- "LC"
...
#############THREATS 
practice <- habitat_table(species_list)
summary(practice)
...
querythreat2 <- sqldf("SELECT * FROM query_threat WHERE `A2.1` >0 ORDER BY category ")

#Seperating the query by category 
querythreat3 <- sqldf("SELECT * FROM query_threat WHERE category == 'VU' ")
querythreat4 <- sqldf("SELECT * FROM query_threat WHERE category == 'EN' ")
querythreat5 <- sqldf("SELECT * FROM query_threat WHERE category == 'CR' ")

querythreat3.1 <- select(querythreat3, -category, -species_list)
x <- colSums(querythreat3.1)
x <- as.data.frame(x)
plot(x)

names <- colnames(querythreat3.1)
sum <- colSums(querythreat3.1)
...
#Correlation  
#(Script for creating heat maps taken from our textbook)
colnames(withoutnames2)
Cordata <- cor(withoutnames2)
Cordata
...
#Correlation Heatmap ~~~
#with the habitat code 
plot4 <- ggplot(dataMelt, aes(x=x, y=y)) + geom_tile (aes(fill = Correlation)) + 
  scale_fill_gradient2(low = muted("red"), mid = "white", high = "blue")+
  labs(title = "Correlation Heat Map of Threat Types") + 
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  xlab("Habitat types") + 
  ylab("Habitat types")
