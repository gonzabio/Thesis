...
##################
rl_threats("Nyctibatrachus dattatreyaensis ", key = token)


#import SE Asian frogs list
#clear out the frogs that I dont have spatial data on 
#map where each frog is based on red list status 



#######SE ASIAN FROGS THREAT ANALYSIS - PROOF OF CONCEPT 
###Goals; 
        #1. SE ASIAN TABLE (has amphBIO, IUCN threats, etc.)
        #2. PCA of threats in SE Asia 
        #3. More exploratory data analysis - do logistic regression again 
        #4. Consider downloading habitat data? 
...
getwd()
setwd("/Users/Alexandra_Gonzalez/Downloads")
se_asian_amph <- read.csv("export-89505.csv", header = TRUE)
summary (se_asian_amph)
...
threat_code2 <- rbind(iucn_threats1, iucn_threats2, iucn_threats3, iucn_threats4,
                     iucn_threats5, iucn_threats6, iucn_threats7)
...
#IUCN data seperates species and genus. AmphibBIO has them together. Code to create
#a new column to make species in IUCN_Data = Genus + species 
new_species2 <- data.frame(A = se_asian_amph$Genus, B = se_asian_amph$Species)
se_asian_amph$new_species <- paste(new_species2$A, new_species2$B, sep=" ")
class(se_asian_amph$new_species)
se_asian_amph$new_species <- as.factor(se_asian_amph$new_species)

#Joins both datasets 
library(sqldf)
se_asian2 <- sqldf("SELECT * FROM se_asian_amph INNER JOIN amphiBIO ON amphiBIO.a_Species = se_asian_amph.new_species")
summary(se_asian2) 
#Joining IUCN and AmphiBIO data together 
...
threat_data2$species_list <- as.character(threat_data2$species_list)
    #see above for how I got to threat_data2
colnames(se_asian2)[colnames(se_asian2) == "a_Species"] <- "species_list"
se_asian2$species_list <- as.character(se_asian2$species_list)
...
hey <- concise_tables(set1)
hey2 <- no_third_level(hey)
